...
`Tuple2[T, Error]`を返す呼び出し式の直後に`?`を書くと、エラーが非nilだった場合その場で(自分を囲む関数から)早期脱出します。AmiFLには汎用の早期脱出構文`return`もありますが(2章参照)、`?`は「ペイロードの取り出し・ゼロ値の組み立て・Error側での早期return」を1トークンにまとめた専用の糖衣で、`Tuple2[T,Error]`/`Error`の伝播という最も頻出するパターンを`return`で手書きするより簡潔に書けます。
