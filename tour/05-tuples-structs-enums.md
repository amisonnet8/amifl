...
`.0`/`.1`を1つずつ書く代わりに、`let (a, b) = ...`でまとめて分解することもできます(`amifl-spec.md` §4)。
...
fn main() -> Int {
    ...
    let (only, _) = (10, "ignored")   // "_"でその位置を明示的に捨てる
    print(only)   // 10
...
名前の個数はタプルの要素数と厳密に一致している必要があります。この構文は`Tuple2`〜`Tuple8`のどれにも使え、6章で扱う`parse[T]`のような`Tuple2[T, Error]`を返す組み込み関数の結果を`let (value, err) = ...`と受けるのに特に便利です。
...
fn main() -> Int {
    ...
    let moved = Point{x: p.x + 1.0, y: p.y}   // 新しい値を作る
    print(moved.x)   // 2

    p.x = 5.0   // フィールドを直接書き換えることもできる
    print(p.x)   // 5
...
`struct`のフィールドは`p.x = 5`で直接書き換えることも、上の`moved`のように新しい値を作り直すこともできます——どちらを選ぶかは自由です。タプルの`.0`等はこれとは対照的に代入できません(タプルは不変値として扱う設計です)。全フィールドが比較可能な型(スカラー・別の`struct`・タプル)なら、`struct`どうしも`==`/`!=`比較ができます。`p.points[i].y = v`のように、添字とフィールドを混在させた多段代入も書けます。
