...
- `&&`/`||`は短絡評価ではありません(2章)——境界外アクセスのリスクがある式は、ネストした`if`で書き直します。
- `Set[T]`の要素順序は不定です(7章)——決定的な順序が必要なら`toList(_) |> sort`と組み合わせます。
- `struct`のフィールド代入構文(`p.x = 5`)はありません(5章)——書き換えたい場合は新しい値を作り直します。
- `String`から`Bytes`への変換関数はありません(8章)——ファイルへ文字列を書く場合、現状は数値のバイト列として明示的に綴る必要があります。
