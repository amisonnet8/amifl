...
`return 5`と`"non-negative"`は一見すると型が違いますが、`return`はどこに現れても制御がそこで発散する(関数の外へ抜ける)ため、周囲がどんな型を期待していても矛盾しません——AmiFLはこれをコンパイラ内部限定の`Never`型として扱います(§2.2)。`break`/`continue`も同じ扱いなので、ループの中でif分岐の末尾として使えます(`if done { break } else { 5 }`)。

## `&&`/`||`の短絡評価(§6)

1章でも触れましたが、制御構文の条件式を書くときにこれが特に効いてきます。`&&`/`||`は短絡評価なので、次のような境界チェックを1つの条件式にまとめても安全です——`j < len(xs)`が`false`なら`xs[j]`はそもそも評価されません。
