...
| `amifl_implementation_notes.md` | AMIVM-IRを生成するフロントエンドを実装する際の実地の知見（踏んだ地雷・確立したパターン）をまとめるメモ。15ステップ全完了＋ロードマップex1〜ex7を通じて蓄積済み（AmiFLが実証したAMIVM命令の一覧、および各ステップで踏んだ地雷とその対策・一般化した教訓を記録）。新しい機能を実装するたびに随時追記していく。Seed/Cascade/Weaveの同名ファイルと同じ位置づけ（各言語の実装知見はその言語自身のリポジトリで管理する方針——`ignored/`配下の参照クローンは更新のたびに丸ごと差し替えられうるため、知見はここに書く） |
...
`amifl-spec.md` 16.1節が正。要約すると、`cmd/amifl/`（CLIエントリポイント）・`internal/{lexer,parser,ast,modloader,sema,codegen}/`（レイヤー分割。`ast`が唯一の共有語彙、`parser`は`lexer`+`ast`に依存、`sema`と`codegen`はどちらも`ast`にのみ依存し互いには依存しない——Seed以来一貫している設計）・`amiflrt/`（独自ランタイム、`go:embed`配布）・`examples/`（実装した構文ごとにサンプルを追加）・`tour/`（言語入門、11章構成のMarkdown。実装完了後に着手済み——詳細は下記「確定した設計判断」の該当節参照）という構成。
...
### ロードマップ5番目の拡張：パッケージ間`struct`/`enum`参照（旧spec 17.3節ex5）——パーサーの汎用な後置`.`連鎖ループが構文面の懸念を丸ごと解消した一方、意味論面で「同じ型が2つの異なる正規名を持つ」という新種の設計課題を要求した

ex4完了後、ユーザーから「ex5を実施。問題なければ完了後コミット。」という指示のみ（ex1〜ex4と同じパターン）で着手。Step 14が`fn`/`const`のみに絞った理由（CLAUDE.md「Step 14のスコープ確定」）は「`alias.Point{...}`が複合リテラルとの構文的曖昧性回避を要り、`alias.Status.Variant(...)`が2段ドットのチェーンを要る」という構文コストだったが、実装してみると**両方とも既存の`parsePostfixExpr`の後置`.`連鎖ループがそのまま生成できる形**だった——`alias.Status.Variant(...)`は単に「`.`が2回続く」だけで、ループが自然に`FieldExpr{Target: FieldExpr{Target: Ident(alias), Field: "Status"}, Field: "Variant", Args: [...]}`という入れ子を組み立てる。新規に要ったのは`alias.Point{...}`（複合リテラル）の1箇所だけ——ループの中で「直前がbareな`*ast.IdentExpr`かつ次のトークンが`{`（`noCompositeLit`が偽の文脈）」を検知して`ast.StructLit`（新設の`Qualifier`フィールド付き）へ切り替える分岐を1つ足すだけで済んだ。構文面はStep 14が懸念していたほど重くなかった一方、**意味論面（sema）には前例の無い新しい設計課題**——「クロスパッケージの同じ型が、参照元によって2つの異なる正規型文字列を持ってしまう」問題——が浮上し、これがex5の実装の大半を占めた。

**確定した設計判断**：

1. **クロスパッケージのstruct/enum型は`"Qualified(GoName)"`という新しい正規型文字列エンベロープで表現し、`c.structs`/`c.enums`マップへ「インポートされた型」として事前登録する**——`GoName`はエクスポート元パッケージ自身の正規プレフィックス込みの完全なGo型名（`buildExports(prefix)`の`prefix`、既存の`ExportedFunc.Token`が`"!"+prefix+name`で使っているのと同じ値）。裸の宣言名（"Point"）をそのまま使わなかった理由は、2つの異なるパッケージが偶然同じ名前のstructを持つ可能性を排除できないため——`Tuple(...)`/`List(...)`等の既存の複合型エンベロープと全く同じ発想（AmiFL識別子は`(`/`)`を含めないため構造的に衝突しない）。sema.goの`CheckPackage`が最初期（Pass 0より前）に`registerImportedTypes`を1回実行し、インポートしている全パッケージの全エクスポート済みstruct/enumを、この`Qualified(...)`キーで`c.structs`/`c.enums`へ登録する——これにより`resolveFieldExpr`の`.field`アクセス・`resolveStructLit`の完全性検査など、既存のstruct/enum関連ロジックのほぼ全てが変更不要のまま、クロスパッケージ型を同じパッケージ内の型と同一の経路で処理できた（唯一の例外が`resolveSwitchExpr`——後述）。
2. **`resolveFieldExpr`に「`alias.EnumType.Variant(...)`」という新しい分岐を追加し、既存の`resolveEnumVariantConstruction`を「表示名」と「正規型文字列」を別々のパラメータとして受け取る形へ拡張した**——同一パッケージの場合は両方とも裸のenum名（変更無し）、クロスパッケージの場合は表示名が`"alias.EnumType"`（エラーメッセージ用）、正規型文字列が`"Qualified(GoName)"`（`v.ResolvedType`として実際に使われる）という非対称な組み合わせになる。この分離により、エラーメッセージは常に読みやすい形（`"mathutil.Status.Retry expects 1 field..."`）を保ちながら、型システム内部は衝突の起きない`Qualified(...)`表現で一貫させられた。
3. **クロスパッケージのenum値を`switch`のサブジェクトにすることは明示的に拒否した**（意図的なスコープ限定）——`resolveSwitchExpr`が`isQualifiedType(subjectTyp)`を検査し、専用のエラーメッセージ（「宣言パッケージ側に検査用関数を用意すること」という代替案を添えて）で早期に拒否する。理由は構文面：`switch`のcaseパターン（`parseSwitchCasePattern`）は同一パッケージ内の裸のenum名しか書けない文法のままで、これを`alias.EnumType.Variant(...)`という3段の形へ拡張するのは新しい専用文法が要る、ex5の本来のスコープ（値の構築・フィールドアクセス・型注釈）を超えた別の作業だと判断した。この制限があっても、クロスパッケージのenum値そのものは（構築・関数への引渡し・フィールド読み取りは効かないが——enumはフィールドを直接晒さない設計、2.2節参照——)完全に無意味にはならない：enum自身の宣言パッケージ側に検査用の関数（内部で`switch`する）を用意して呼び出す、という素直な回避策が常に使える。
4. **`resolveQualifiedReference`（既存のクロスパッケージ`fn`/`const`解決関数）に、名前がstruct/enum型を指している場合の専用エラーメッセージを追加した**——「`mathutil.Point`は型であって値ではない、`mathutil.Point{...}`で構築するか型注釈で使うこと」という形。これはex5が無くても発生しうる状況（`let x = mathutil.Point`のような裸の型名参照）に対する、既存の「no exported name」という汎用エラーより親切な診断を追加しただけで、既存のロジック自体には手を加えていない。
5. **最大の設計課題：`buildExports`が返す型文字列（関数のパラメータ/戻り値型、structのフィールド型、enumバリアントのフィールド型）は、エクスポート元パッケージ自身の視点（裸の"Point"）ではなく、インポート先パッケージの視点（`Qualified(prefix+"Point")`）へ変換してからエクスポートする必要がある——`exportTypeString`という再帰的な型文字列書き換え関数を新設して解決した。** 最初の実装（構築構文とstruct/enum型の登録のみ）を実地検証したところ、`geo.SumPoint(p)`（`p`は`geo.Point{...}`で構築した値、`SumPoint`は`geo`自身が`fn SumPoint(p: Point) -> Int`と宣言）が`"expected Point, got Qualified(geo_Point)"`という型エラーで拒否された——`SumPoint`のパラメータ型は`geo`自身の`c.funcs`に**裸の**"Point"として登録されている（`geo`自身の内部では正しい）が、`buildExports`はこれをそのまま`ExportedFunc.Params`へコピーしていたため、インポート元（呼び出し側）から見ると「Point」という、自分の`c.structs`に存在しないキーになっていた。対策は`exportTypeString(t, prefix)`——`t`がローカルなstruct/enum名なら`Qualified(prefix+t)`へ書き換え、`Tuple`/`List`/`Array`/`Set`/`Map`/`Chan`/`Stream`/`Func`のような複合型なら各要素へ再帰する（`splitTopLevelCommas`等の既存の型文字列分解ヘルパーをそのまま利用）——を`buildExports`が`ExportedFunc.Params`/`.Ret`・`ExportedConst.Typ`・`ExportedStruct.Fields[].Typ`・`ExportedEnum.Variants[].Fields[].Typ`の**全て**に適用する形にした。既に`Qualified(...)`である型（そのパッケージ自身が別のパッケージから間接的にインポートした型）は素通しする——GoNameがどのインポート元から見ても不変であるため、これ以上の変換が不要という性質を利用している。
6. **さらに1段深い問題：codegenの複合型キャッシュ（`listGoTypeName`等）が「正規型文字列」をキャッシュキーに使っていたため、同じGo型が2つの異なるキー（"List(Point)" vs "List(Qualified(geo_Point))"）の下で2回ミントされてしまう問題を発見・修正した。** 上記5の修正後も、`List[Point]`（`geo`自身の内部の型注釈、裸の"Point"を含む）と`List[geo.Point]`（インポート元での型注釈、`Qualified(geo_Point)`を含む）が同じGo要素型（`geo_Point`）を指しているにもかかわらず、`listGoTypeName`が正規型文字列そのもの（"List(Point)" vs "List(Qualified(geo_Point))"）をキャッシュキーにしていたため、**2つの構造的に同一だが別々に発行されたGoスライス型**（`AmiflList1`と`AmiflList2`）が生成され、`go build`が「代入不可能」として拒否した——ex3の`Func`型が踏んだ「Goの名前付き型は構造の一致だけでは代入可能にならない」という罠の、複合型キャッシュ版の再演。対策は、`listGoTypeName`/`arrayGoTypeName`/`setGoTypeName`/`mapGoTypeName`/`chanGoTypeName`/`streamGoTypeName`/`tupleGoTypeName`/`funcGoTypeName`という**8つ全ての**Go型ミント関数について、キャッシュキーを「入力の正規型文字列そのもの」から「要素/フィールドを`resolveGoType`で解決した*後*のGo型名を使って再構築したキー」へ切り替えたこと——例えば`listGoTypeName`は`elemGoType := p.resolveGoType(listElemType(canonical))`を先に計算し、`"List(" + elemGoType + ")"`をキャッシュキーとして使う。これは正しい設計变更である理由がある：sema側で2つの正規型文字列が異なっていても、それらが指す**Go型が同じ**であれば、Go自身にとっては同じ型として扱われるべきであり（AmiFLレベルの型安全性は既にsemaが保証済みなので、codegenがGo型を過剰に区別する必要は無い）、この統一はネストした複合型（`List[List[Point]]`等）にも再帰的に効く（内側の型のGo名が既に統一されているため、外側のキャッシュキーも自動的に統一される）。

**実装は7ファイル**：`internal/ast`（`ast.QualifiedType`ノード新設、`ast.StructLit.Qualifier`フィールド追加）、`internal/parser`（`parseTypeExpr`冒頭の`alias.Name`分岐、`parsePostfixExpr`の`.`ループ内の`alias.Name{...}`検知分岐）、`internal/sema`（`module.go`の`Exports.Structs`/`.Enums`＋`buildExports`の拡張＋`exportTypeString`＋`registerImportedTypes`新設、`sema.go`の`CheckPackage`冒頭での呼び出し、`expr.go`の`resolveTypeExpr`/`resolveStructLit`/`resolveFieldExpr`/`resolveEnumVariantConstruction`/`resolveSwitchExpr`の拡張、`types.go`の`makeQualifiedType`/`isQualifiedType`/`qualifiedTypeGoName`新設）、`internal/codegen`（`structs.go`の`resolveGoType`への`isQualifiedType`分岐＋同名ヘルパーの独立コピー、8つのGo型ミント関数のキャッシュキー変更）。新しいAMIVM命令は一切不要——既存の`STTYPE`/`FIELD`/`FGET`/`FSET`がそのまま使える。

**実地検証（`amivm`→`go build`→実行）で確認した項目**：使い捨ての2パッケージ構成（`geo`パッケージがstruct `Point`・enum `Shape`・関連関数をエクスポート、`main`が両方を構築・フィールドアクセス・関数呼び出し）を`amivm`→`go build`→実行まで通し、終了コード61（`SumPoint(構築したPoint)=7` + `MakePoint(10,20)のフィールド和=30` + `Area(Circle)=12` + `Area(Rect)=12`）が手計算と一致することを確認——これが上記5のバグ（`SumPoint`呼び出しの型エラー）を最初に発見した実行でもある。修正後、さらに`List[geo.Point]`を経由するstructのネストケース（上記6のバグ）も別の使い捨てサンプルで発見・修正し、終了コード3（`p.points[0]`のx+y）で確認した。既存の`examples/modules`（Step 14からの唯一の複数パッケージサンプル）へ`Interval`（struct）・`ClampOutcome`（enum、`Unchanged`/`Adjusted(delta: Int)`）を追加する形で拡張し（新規ファイルを作らなかった理由はex4と同じ——既存の「モジュール」機能そのものの直接の拡張であるため）、`mathutil`パッケージ内で`switch`によるenum検査を完結させつつ`main`側は構築・型注釈・フィールドアクセスのみを行う設計にした（設計判断3のとおり、クロスパッケージ`switch`は使えないため）。終了コード137（既存の132に`e(10)+f(-5)+g(0)=5`を加算）が手計算と一致することを確認。加えて、フィールド不足のクロスパッケージstruct構築・未知のstruct/enum名・未定義のインポートエイリアス・クロスパッケージ`switch`・裸のクロスパッケージ型名を値として参照、の5パターンを手動でCLIから確認し、いずれも期待どおり明確なエラーメッセージでコンパイルエラーになることを確認した。`internal/parser/parser_test.go`（6件）・`internal/sema/sema_test.go`（14件、`qt`ヘルパー新設）・`internal/codegen/codegen_test.go`（4件、うち1件は上記6のバグの直接の回帰テスト）に回帰テストを追加。`gofmt`/`go vet`/`go test`/`make test-examples`はすべてgreenで、既存`examples/`全30ファイルに回帰が無いことも確認済み。`amifl-spec.md`の12.2/17.1/17.2/17.3節・冒頭注記も実装に合わせて更新した。**これでロードマップ6項目のうち5項目（ex1〜ex5）が完了し、残るはex6（`print`/`eprint`/`format`/`formatWith`/`exit`実装）のみとなった。**

### ロードマップ6番目（最終項目）の拡張：`print`の`Any`一般化＋`eprint`/`format`/`formatWith`/`exit`（旧spec 17.3節ex6）——新しい型システム機構は一切不要で、既存の3つの前例（typeNameのAnyバイパス、Tap/Peekの%v書式、`!main`のint64→intキャスト）を組み合わせるだけで完結した

ex5完了後、ユーザーから「ex6を実施。問題なければ完了後コミット。」という指示のみ（ex1〜ex5と同じパターン）で着手。この時点でロードマップメモ自身が挙げていた設計上の懸念は主に`exit`の型付け（Never的な「あらゆる期待型に適合する」設計が必要か）だったが、着手前の調査で「AmiFLには`Any`という型注釈自体が`extern`bindの引数/戻り値位置にしか存在しない（`canonicalType`が一般には`Any`を認識しない、design issue 1の確定判断）」という既存の制約を再確認したことで、`formatWith`の設計（当初検討した`List[Any]`/`TupleN`ベースの複数値版）も含め、**全ての新機能を「単一の`Any`型引数を持つ、印刷済みの前例（`print`/`typeName`/`isError`）と同じ形」に落とし込む**という方針に自然に収束した。

**確定した設計判断**：

1. **`print(v: Any) -> Unit`への一般化は、sema側1箇所の変更だけで完結し、codegen側は無変更だった**——`resolveCallExpr`の`print`専用分岐（expr.go、step 1以来変更されていない箇所）で、`checkExprPipeAware(v, 0, v.Args[0], "String")`を`"Any"`へ差し替え、`argTyp == unitType`の明示的な拒否を追加しただけ（`for...yield`のYield・`tap`/`peek`のv・Step 9/15で確立済みの「Unit型はGoランタイム表現を持たないので明示的に弾く」という定型パターンの再利用）。codegen側（codegen.goの`genCallStmt`、`c.Callee == "print"`分岐）は一切変更していない——Goの`fmt.Println(a ...any)`は元々`...any`可変長引数で、呼び出し時に渡された値が`Int64`だろうと`struct`だろうと、Goの暗黙のインターフェース変換だけでそのまま`any`へ収まる（Step 13の`extern`Any境界実装が発見した「Any値を*受け取る*方向は変換コード不要」という結論の直接の再利用）。`typeName`が必要とした`boxIntLiteralForAny`（裸の整数リテラルが`%T`で`"int64"`ではなく`"int"`になる罠）はここでは不要——その罠は`%T`（動的型名の取得）特有のものであり、`%v`（値の表示）は数値の内部幅に関わらず同じ文字列を出力するため。
2. **`eprint`/`format`/`formatWith`は、Tap/Peekが確立した「amiflrtへ委譲する単純な`v any`関数」パターンに乗せたが、Go側の型パラメータは1つも要らなかった**——Tap/Peek（step 15）は`v`をそのまま返す（＝戻り値の型が`v`自身の型と一致する必要がある）ためGoジェネリクス`T any`が必須だったが、eprint（`Unit`を返す）・format/formatWith（`String`を返す）はいずれも`v`を読み取るだけで返さない——戻り値の型が`v`の型に依存しないため、`amiflrt.Eprint(v any)`/`Format(v any) string`/`FormatWith(template string, v any) string`という**非ジェネリックな**Go関数で足りた（typeNameの`fmt.Sprintf("%T", v)`が非ジェネリックで済んでいたのと同じ理由）。codegen側もTap/Peekの`writeGenericCall`ではなく、typeNameと同じ素の`writeCall`（型引数区画なし）で済んだ。
3. **`formatWith`は複数値版（`List[Any]`または`TupleN[...]`引数）を検討したが、単一`Any`値＋`|>`連鎖という設計に倒した**——最初に検討した`formatWith(template: String, args: List[Any]) -> String`は、AmiFLの`List[T]`が常に単一の具体型`T`を持つ（ヘテロジニアスなリスト値を構築する構文自体が存在しない）ことと、design issue 1が確定させた「`Any`はAmiFLの一般的な型注釈文法には一度も現れない、`extern`境界専用の特殊ケース」という制約に正面から抵触する——`List[Any]`という型注釈自体をユーザーが書く手段が無い。次に検討した`TupleN[...]`ベース（Tuple2〜8の各フィールドを個別に`Any`境界越しにamiflrtの可変長`...any`パラメータへ渡す設計）は技術的には実現可能だったが、Tuple2〜8という7つの別々のアリティを1つの組み込み関数名の下でcapability的にディスパッチする仕組みを新設する必要があり、「1つの仕組みで足りるものを2つ用意しない」（原則3）の観点で見ると、**AmiFLが既に持つ`|>`という「値を連鎖させる」唯一の仕組みをそのまま再利用する**方がよほど単純だと判断した：`formatWith(template, v)`は最も左（先頭）の`{}`だけを1回埋めて`String`を返す設計にし、2箇所以上埋めたい場合は`"{} and {}" |> formatWith(_, a) |> formatWith(_, b)`のように連鎖させる——各呼び出しが「残っている最も左の`{}`」を消費するため、この連鎖は素直に正しく動作する（実地確認済み、下記参照）。新しい組み込み関数の可変長引数的な特別扱いも、Tuple向けのディスパッチロジックも一切不要だった。
4. **`exit(code: Int) -> Unit`は、Goの`panic`のような「あらゆる期待型に構造的に適合するNever型」にはしなかった**——ロードマップメモ自身が挙げていた設計上の分岐点（design-issue的な検討）を、実装前の調査で「そもそもAmiFLの型解決パイプラインが`expected`をCallExprの型決定へ渡さない」という構造的な事実に基づいて決着させた：`resolveType`のCallExprケースは常に`fc.resolveCallExpr(v)`を`expected`抜きで呼び、CallExpr自身が確定させた型を後から`checkExpr`が`expected`と突き合わせるだけ——`unwrap[T]`が「Tは引数`Tuple2[T,Error]`自身の`T`から取り出す」という形でこの制約を回避しているのとは違い、`exit`には「戻り値の型を教えてくれる引数」が無い（`exit(1)`の`1`はcode自身であって型ヒントではない）。`expected`を一般のCallExpr解決パイプラインへ新たに通す変更は、単一の組み込み関数のためだけに行うには構造的に重すぎる（CLAUDE.mdの開発方針8番が想定する「大きな変更」寄り）と判断し、**`exit`は素直に`Unit`型固定とし、`print`/`eprint`と同じ「Unitが許される場所でのみ使える」制約を受け入れた**——`if ok { 5 } else { exit(1) }`（非Unit分岐の代わりに使う、Goのpanicなら通る書き方）はコンパイルエラーになるが、`if bad { eprint("fatal"); exit(1) } else { ... }`（両分岐がUnit）は問題なく書ける。amifl-spec.md 17.2節#11に既知の限界として明記した。
5. **`exit`のコード生成は、`!main`ラッパー自身が確立した「Int64→native int」キャスト（step 1）をそのまま複製した**——`os.Exit`はGoのプラットフォーム依存`int`を要求するが、AmiFLの`Int`は固定幅`Int64`——`!main`ラッパー（codegen.goの`GenerateProgram`）が最初から`CALL %exitCode : ?int %code`という「CALLをGoの型変換として使う」トリック（CLAUDE.mdの「過去に踏まれた地雷」#5）で解決していたのと全く同じ変換を、`exit`組み込み自身のコード生成（`internal/codegen/output.go`の`genExitStmt`）でも1回挟むだけで済んだ——新しい変換ロジックの発明は不要だった。

**実装は4ファイル+2箇所の登録**：`internal/sema/builtins_output.go`（新設、`resolveEprint`/`resolveFormat`/`resolveFormatWith`/`resolveExit`、`builtins_pipeline.go`と同じ「対象セクション専用ファイル＋自身の`init()`でbuiltinFuncsへ登録」という既存パターンを踏襲）、`internal/sema/expr.go`（`print`専用分岐のAny化+Unit拒否）、`internal/sema/builtins.go`（eprint/format/formatWith/exitの`notYetImplemented`プレースホルダーを削除——4つとも今や`builtins_output.go`の`init()`が登録するため、ここに直接列挙する必要が無くなった）、`amiflrt/output.go`（新設、`Eprint`/`Format`/`FormatWith`の3つの非ジェネリック関数）、`internal/codegen/output.go`（新設、`genEprintStmt`/`genFormatValue`/`genFormatWithValue`/`genExitStmt`）、`internal/codegen/builtins.go`（`genBuiltinStmt`/`genBuiltinValue`スイッチへのディスパッチ追加）。新しいAMIVM命令もamivm本体への機能要求も一切不要——既存の`CALL`だけで完結した。

**実地検証（`amivm`→`go build`→実行）で確認した項目**：新規`examples/output_and_exit.aml`——`print`へInt/Bool/struct（`Point{x:1,y:2}`）/Tuple（`(1,"two")`）を渡す（従来の`print("...")`しか書けなかったコードがそのまま動き続けることも同時に確認）、`eprint`が標準出力ではなく標準エラー出力へ書くこと（プロセスのstdout/stderrを別々のファイルへリダイレクトして目視確認）、`format(3.5)`が`"3.5"`という文字列を返すこと、`"point: ({}, {})" |> formatWith(_, 10) |> formatWith(_, 20)`が`"point: (10, 20)"`（左から順に2箇所とも正しく埋まる）になること、`exit(77)`が両分岐Unit型の`if`の中から実際にプロセスを終了コード77で終わらせること（`if`の直後に到達不能な`99`という末尾式を意図的に置き、もし`exit`が正しく即座終了していなければ終了コードが77ではなく99になってしまうという、実行してみて初めて意味を持つ検証）。実行結果は標準出力6行・標準エラー出力1行・終了コード77がすべて期待どおりであることを確認。生成されたIRも目視確認し、`print`が`?fmt.Println`直呼び（Any化前と1文字も変わらない形）、`eprint`/`format`/`formatWith`が`?amiflrt.Eprint`/`.Format`/`.FormatWith`への単純な`CALL`、`exit`が`CALL ?int`→`CALL ?os.Exit`という`!main`ラッパーと同型の2段になっていることを確認した。既存の`examples/pipe.aml`（終了コード178）・`examples/enums.aml`（22）・`examples/tuples_structs.aml`（40）・`examples/data_ops.aml`（46）・`examples/modules`（137）を再実行し、`print`のAny化が既存の全`String`専用呼び出しに対して1つも回帰を起こしていないことを確認した。`internal/sema/sema_test.go`に10件（print系3件・eprint系2件・format系2件・formatWith系1件・exit系2件のうち`if`分岐の型付け境界を明示的に確認する2件を含む）、`internal/codegen/codegen_test.go`に5件を追加。`gofmt`/`go vet`/`go test`/`make test-examples`はすべてgreenで、既存`examples/`全31ファイル（新規`output_and_exit.aml`含む）に回帰が無いことも確認済み。`amifl-spec.md`の13.1/17.1/17.2/17.3節・冒頭注記も実装に合わせて更新した。**これでロードマップ6項目全て（ex1〜ex6）が完了した。**

### ロードマップ7番目の拡張：16進/8進/2進数値リテラル・桁区切り`_`（旧spec 17.3節、ユーザーが新たに追加した7番目の項目）——コンパイラ側の変更は字句解析＋パーサーの2層に閉じ、sema・codegenの意味論的なロジックは1行も変わらなかった

ex6完了・6項目のロードマップ完走の後、ユーザーから新たに7番目の項目として提案・着手指示があった（「ex7として 16進/8進/2進数値リテラル、桁区切り`_`の追加を実施します。AMIVMも...追加を行いました...新バージョンのAIMVMはインストール済み。ex7を実施。問題なければ完了後コミット。」）——`amivm`本体も同じタイミングでこの機能に対応する形で先にアップグレードされていた（`ignored/amivm/amivm_spec.md`6節、`ignored/amivm`のコミット`22c701f`）。

**確定した設計判断**：

1. **AmiFLの`ast.IntLit`に生の字句テキストを保持する`Token`フィールドを追加し、codegenは`Value`（パース済みのuint64、常に基数非依存の大きさそのもの）からの再構成ではなく`Token`をそのまま出力する設計にした**——`amivm`の新しい整数リテラル文法（`ignored/amivm/amivm_spec.md`6節）は`0x1A`/`0o17`/`0b101`/`1_000_000`のようなトークンを「そのまま」Goの数値リテラル構文として受理する（Goのネイティブ数値リテラル構文がGo 1.13以降まさにこの形を持つため）。既存のStep3の設計（`-`単項マイナスはリテラルトークンへ直接符号を前置するだけで専用命令を持たない、CLAUDE.mdの「確定した設計判断」参照）と方向性を合わせ、「ユーザーが書いた表記をできる限りそのままIRへ運ぶ」という一貫性を保った——`literalToken`（codegen.go、単項マイナスの畳み込みに使う既存ヘルパー）は変更不要で、`"-" + 生トークン`という既存の文字列結合ロジックがそのまま`"-0x1A"`という、amivm自身が6節で明示的に例示している負のリテラル形をそのまま生成した。**`Token`はパーサーが構築する場合にのみ設定され、空文字列なら`Value`から10進数を再構成するフォールバックを`intLitToken`ヘルパーに持たせた**——コンパイラ内部で（テストコード以外に）直接`ast.IntLit{}`を組み立てる箇所は`internal/parser`しか無かったが、何百とある既存テストの手組み`&ast.IntLit{Value: N}`（`Token`未設定）を1つも書き換えずに済ませるための後方互換フォールバックとして必須だった。
2. **浮動小数点リテラルには桁区切り`_`だけを追加し、`Token`によるトークン直輸送の対象には含めなかった**——`amivm`自身の6節を精査すると、桁区切り`_`は整数リテラルの行にのみ記載されており、浮動小数点の行（`123.4` / `1.23e4`、そのまま）には記載が無い。一方、AmiFLのfloat codegen（`formatFloatLit`）はStep3以来ずっと`Value`（`float64`）から`strconv.FormatFloat`で再構成する設計であり、生のソーステキストを一度も保持・パススルーしていない——つまり浮動小数点リテラルは、コンパイラ内部で最初から「値そのもの」だけを運ぶ設計になっている。Goの`strconv.ParseFloat`自体はGoの浮動小数点リテラル構文に沿って`_`を受理する（実地確認済み、下記参照）ため、AmiFLの字句解析側だけ`_`を浮動小数点リテラルにも許可しつつ、`amivm`へ渡す段階では常に`_`の抜けた10進の`Value`が再構成されて運ばれる——ユーザー向けの読みやすさ（`1_000.5`と書ける）と、`amivm`側の文法をこれ以上広げないことの両方を満たせる設計にした。`amifl-spec.md`17.2節#13に明記。
3. **`strconv.ParseUint(text, 0, 64)`（base 0＝プレフィックスから基数を自動判定）へ切り替えるにあたり、Goの「裸の`0`プレフィックスはレガシー8進数として解釈される」という仕様が正しさの罠になることを実装前に発見し、字句解析側で先回りして防いだ**——`amivm`自身の6節が「10進リテラルは`0`単体、または`1`-`9`に`0`-`9`が続く形のみ」と厳格に制限している理由がまさにこれだった：もし字句解析が`017`のような先頭ゼロ付き10進表記をそのまま1トークンとして許し、パーサーがbase 0の`ParseUint`へそのまま渡すと、Goは`017`を**8進数の15**として解釈してしまう（10進の17ではなく）——ユーザーの意図と全く異なる値が、エラーも警告も無く静かに生成されるという、最も危険な種類のバグになる。この罠を`lexNumber`（lexer.go）が実際にトークンを切り出す時点で「`0`で始まり、かつ2文字目以降に数字または`_`が続く10進表記」を明示的に拒否する形で防いだ——浮動小数点になる場合（`017.5`のような形。float側はGoの`ParseFloat`が8進数レガシー扱いを一切しないため危険が無い）だけは、この拒否判定を`.`/指数部の判定が済んだ**後**まで遅延させることで誤検出を避けた（「`0`が浮動小数点の整数部の場合は制限しない」という非対称なルール）。
4. **基数プレフィックス後の桁の妥当性検証（8進で`8`/`9`が来る、2進で`2`-`9`が来る、等）は字句解析側で一切行わず、`strconv.ParseUint`自身のエラー報告に委ねた**——`0o18`（8は8進数として不正）のような入力を、字句解析が「`0o1`と`8`という2つの別トークンに分割する」のではなく、「`0o18`という1つの（無効な）トークンとして切り出し、後段の`strconv.ParseUint`が`invalid syntax`エラーを返す」という設計にした。前者（分割）だとユーザーには「なぜ`8`が単独で浮いているのか」という不可解なエラーになりかねないが、後者なら「このリテラルトークン全体が不正」という一目で分かる形でエラーが出る。桁区切り`_`の配置規則（先頭・末尾・連続禁止）についても全く同じ理由で字句解析側での検証を行わず、`strconv.ParseUint`/`ParseFloat`自身の検証（Go 1.13以降、桁区切り`_`の配置規則を厳密にチェックする）にそのまま委ねた——実地確認（下記）で「`1__0`」「`_10`」「`10_`」がいずれも正しく`invalid syntax`エラーになることを確認済み。

**実装は3ファイル**：`internal/lexer/lexer.go`（`lexNumber`の全面書き換え——プレフィックス判定・桁区切り`_`の許容・先頭ゼロ拒否、新設`consumeDigitsAndUnderscores`/`isHexDigit`ヘルパー、`exponentEnd`への`_`許容追加）、`internal/parser/parser.go`（`strconv.ParseUint`の基数を`10`固定から`0`（自動判定）へ、`ast.IntLit`構築時に`Token`フィールドを追加）、`internal/ast/ast.go`（`IntLit.Token`フィールド新設）、`internal/codegen/codegen.go`（新設`intLitToken`ヘルパー、`genValue`と`literalToken`の両方のIntLit分岐をこれ経由に統一）。**sema側は1行も変更していない**——`resolveIntLit`/`resolveNegatedIntLit`のオーバーフロー検査はいずれも既に`Value`（uint64、基数非依存）だけを見ているため、この値がどの基数で書かれたソーステキストから来たかを一切気にする必要が無く、ex7による影響範囲から構造的に除外されていた。新しいAMIVM命令もamivm本体への追加機能要求も不要——今回は逆に、amivm側が先にこの機能を追加し終えていた（ユーザーからの事前アップグレード）。

**実装中に発見した罠（設計の誤りではなく、環境の罠）**：ユーザーから「新バージョンの`amivm`はインストール済み」という申し送りを受けていたにもかかわらず、実装完了後に`examples/numeric_literals.aml`を`amivm`→`go build`まで通したところ、`SET %hexMask_1 0xFF`という最も基本的な形ですら`IR parse error: ... unparseable format for value: 0xFF`という、新機能が全く効いていないかのようなエラーで失敗した。`ignored/amivm`の`git log`を確認すると該当コミット（`22c701f`）は既に`origin/main`に取り込まれ、ローカルの`ignored/amivm`チェックアウトも`git status`上は最新だったため、一見「バージョンの問題ではない」ように見えた——しかし`ignored/amivm`から直接`go build`した使い捨てバイナリで同じIRを試したところ、この機能が正しく動作することを確認できた。`md5sum`で比較したところ、実際に`$PATH`上にインストールされていた`/go/bin/amivm`（インストール時刻は該当コミットの直後だった）は、ローカルの最新チェックアウトから今ビルドしたバイナリと**ハッシュが異なっていた**——つまり「インストール済み」という申し送りと実際にインストールされていたバイナリの中身が食い違っていた（`go install <module>@latest`がモジュールプロキシのキャッシュ等の事情で本当に最新のコミットを反映できていなかった可能性が高いが、根本原因の特定までは行っていない）。対策は`ignored/amivm`ディレクトリで`make install`（ローカルチェックアウトから直接ビルド・インストール）を実行し、インストール後のハッシュがローカルビルドと一致することを確認してから実装検証をやり直したこと。**「ユーザーから最新版がインストール済みと申し送りを受けていても、実地検証（`amivm`→`go build`→実行）が想定外の失敗をした場合は、まず自分のコンパイラ実装ではなく`amivm`側の実際のインストール状態を疑うこと」——CLAUDE.mdが繰り返し強調してきた「ロジック上正しそうに見えるだけで次のステップへ進まない」という原則の、今回は自分のコード以外の場所（外部ツールのインストール状態）にまで適用範囲が及んだ実例として`amifl_implementation_notes.md`にも記録する。**

**実地検証（`amivm`→`go build`→実行）で確認した項目**：新規`examples/numeric_literals.aml`——16進（`0xFF`、`hexMask & 0x0_F`という桁区切り付きの16進マスク演算）・8進（`0o17`）・2進（`0b1010`）の整数リテラル、桁区切り`_`付きの大きな10進数（`1_000_000`）とその除算、桁区切り`_`付きの浮動小数点（`1_000.5`）、いずれの基数にも前置できる負のリテラル（`-0x10`・`-0o10`・`-0b1000`）。生成されたIRを目視確認し、整数リテラルが元のソーステキストのまま（`0xFF`・`0x0_F`・`0o17`・`0b1010`・`1_000_000`・`1_000`・`-0x10`・`-0o10`・`-0b1000`）出力されている一方、浮動小数点リテラルは設計判断2のとおり`1000.5`（桁区切り無し、`Value`からの再構成）へ変換されていることを確認した。実行結果は標準出力9行（`255 / 15 / 15 / 10 / 1000 / -16 / -8 / -8 / 1000.5`）・終了コード240（`15+15+10+1000-16-8-8=1008`、256の剰余で240）が手計算と一致することを確認。加えて、先頭ゼロ付き10進表記（`017`・`0_5`）・8進数として不正な桁（`0o18`）の3パターンを手動でCLIから確認し、いずれも期待どおり明確なエラーメッセージでコンパイルエラーになることを確認した。`internal/lexer/lexer_test.go`（8件）・`internal/parser/parser_test.go`（2件）・`internal/codegen/codegen_test.go`（3件）に回帰テストを追加。`gofmt`/`go vet`/`go test`/`make test-examples`はすべてgreenで、既存`examples/`全32ファイル（新規`numeric_literals.aml`含む）に回帰が無いことも確認済み（`ignored/amivm`の再インストール後に再確認）。`amifl-spec.md`の3.1/17.1/17.2/17.3節・冒頭注記も実装に合わせて更新した。

### 入門ガイド`tour/`を新設——`ignored/weave/tour/`のフォーマットをそのまま踏襲し、全コード例を実地検証した

15ステップ計画＋ex1〜ex7が全て完了した後、ユーザーから「AmiFL入門ガイド(tour/)を追加。`ignored/weave/tour/`を参考に。AmiFLの基礎。AmiFLらしさが学べるように。」という指示を受けて着手した。`README.md`（目次・実行環境）＋`00-welcome.md`〜`10-wrapup.md`の11ファイル構成——Weaveのツアー（`00-welcome.md`〜`09-wrapup.md`の10ファイル）と同じ「1章1ファイル、各章末に演習＋`<details>`で畳んだ解答例、`amifl-spec.md`の節番号を`§4.2`のような形で随所に添える」という書式をそのまま踏襲した。

**章立ては`amifl-spec.md`の構成に沿いつつ、「AmiFLらしさ」が伝わる順序に組み替えた**：0(ようこそ・7原則)→1(スカラー型・`let`/`const`)→2(制御構文・`switch`Bool専用形)→3(関数・クロージャー・`Func`型)→4(`Array`/`List`/`Range`/`for`)→5(タプル・`struct`・`enum`・`switch`パターンマッチ)→6(パイプ演算子・エラー処理)→7(`Set`/`Map`)→8(並列処理・ファイルI/O)→9(`extern`・モジュール)→10(まとめ)。6章（パイプ演算子＋`Tuple2[T,Error]`＋`?`演算子＋capability多相の紹介）を単独の章として独立させたのは、AmiFLの「AWK/Perlの系譜だがPerl的な乱雑さを避ける」という位置づけ（§0）が最も濃く表れる箇所であり、既存の章立てのどこか1つに埋没させず主役として扱うべきと判断したため——Weaveのツアーがアクターモデル・プロトタイプOOPをそれぞれ独立章にしているのと同じ「その言語固有の核心機能は単独章に格上げする」という判断基準を踏襲した。

**全てのコード例を実際に`amifl build`→実行まで通してから本文へ採用した**（このプロジェクト全体を貫く「実地検証必須」の原則を、ドキュメント執筆でも省略しなかった）。この過程で、ツアー執筆前には気づいていなかった実装の実際の挙動を3件発見し、本文をその場で訂正した：

1. **`switch`の非網羅ケースが実際にコンパイルエラーになることを確認**（5章の説明用コード、`Status.Failed`のcaseを意図的に落とした例）——`amifl build`に通し、`switch over Status is not exhaustive and has no default (missing: Failed)`という実際のエラーメッセージを確認してから「コンパイルエラーになる」という記述を確定させた。
2. **`Point{x:1.0,y:2.0}`のような`Float`フィールドを`print`すると、Goの`%v`書式により`"1"`(末尾の`.0`が付かない)として出力される**——執筆時点の思い込みで`// 1.0`とコメントしていた箇所を、実行結果`1`に合わせて訂正した。
3. **式の途中に改行を挟めない、という制約がブロック直下限定ではなく式全体に及ぶことを、リストリテラル・関数呼び出し引数・パイプチェーンそれぞれで個別に実地確認して発見した**——`let xs: List[Int] = [\n    1,\n    2,\n]`のような、他の多くの言語なら自然に書ける複数行リストリテラルが`unexpected newline, expected an expression`という構文エラーになることを、ツアー本文執筆中に(パイプ演算子の章で複数行のパイプチェーンを書こうとして)偶然発見した。既存の`examples/`が(意図的にではなく、結果的に)この形を一度も踏んでいなかったため、これまで文書化されていなかった実際の挙動——このセッションで初めて`amifl-spec.md` §5へ追記した（上記「式の区切り」の節参照）。1章に「1つの式は必ず1行で書き切る」という専用の注意書きを新設し、6章のパイプ演算子の节ではこの1章の説明を参照する形にとどめた（同じ制約を章ごとに重複して説明しない）。

**8章（並列処理・ファイルI/O）の`Bytes`書き込み例は、既知の限界（`String`→`Bytes`の変換関数が無い、§17.2）を隠さず、数値のバイト列をそのまま書く形で示した**——ツアーが「できないこと」を誤魔化さない、CLAUDE.mdの開発方針14番（`weave_spec.md`由来の教訓だが、AmiFLでも同じ精神で運用している）を踏襲した判断。9章の演習では、`fn area`をわざと小文字始まりのまま書いて実際に`package "shapes" has no exported name "area"`というエラーになることを確認してから、大文字始まりへ直す形の解答例にした——可視性規則(§12.2)を「読んで理解する」だけでなく「実際に踏んでみて理解する」演習にする狙い。

`gofmt`/`go vet`/`go test`/`make test-examples`の回帰は今回のツアー追加では対象外（Markdownのみの追加でコンパイラ本体への変更を伴わないため）だが、上記の§5追記・equality修正（直前の「examples/spec見直し」節参照）を含む全変更を通しで`go test ./...`／`make test-examples`していずれもgreenであることを確認した。

### ロードマップ8番目の拡張：カンマ区切りリテラル/引数リストの複数行対応と`|>`チェーンの行またぎ継続（ex8）——ツアー執筆中に発見した「1行1式」制約（直前の節参照）を、ユーザー自身の指摘で正式に緩和した

tourを一通り読んだユーザーから「イケてない」箇所として、複数行リストリテラルと複数行パイプチェーンが書けない制約の緩和を明示的に依頼された。着手前にユーザーへ提示した論点は2つ：(a) `[...]`/`(...)`の中で改行を無視するのは「括弧の深さを数える」だけで対応できるが、`|>`を次の行の先頭に置く継続は**深さ0**で起きるため全く別の仕組み（先読み）が要ること、(b) `{`がブロックとstructリテラルの二重の役割を持つため、括弧の深さと`{`の深さを混同すると将来`(...)`の中にクロージャー本体の`{ }`が来た場合に改行の意味を誤って奪いかねないこと——実装してみるとこの2点はほぼそのまま実装のボトルネックになった。

**確定した設計判断**：

1. **カンマ区切りリストは共通ヘルパー`parseCommaList[T any]`（`internal/parser/parser.go`）に統一した**——開き括弧を消費済みの状態から、closeトークンに達するまで`parseOne`を繰り返し呼び、要素の後・カンマの後に`skipNewlines()`を挟む。ループ条件が「cur == close」なので、末尾カンマ（`[1,\n2,\n]`の最後の`,`）も自然に許容される——「カンマの次が要素」ではなく「カンマの次がcloseなら即終了」という判定が、そのまま末尾カンマ対応になる。`parseTupleType`（`Tuple2[...]`型注釈）・`parseFuncType`（`fn(...)->R`型注釈のパラメータ）・`parseArrayType`（`Array[T;N1,N2,...]`のサイズ列）・`parseFieldTypeList`（`struct`宣言のフィールド列、`fn`のパラメータ列）・`parseFieldCallArgs`（enum構築/qualified呼び出しの引数）・`parseListLit`（`List`/`Array`リテラル）・`parseSwitchCasePattern`のバインディング列・`parseCallArgs`（通常の呼び出し引数）・`parseStructLit`（structリテラルのフィールド）の9箇所をこのヘルパーへ統一した。`parseParenOrTupleExpr`（タプル/グルーピングの判別が絡む）・`parseBraceLit`（Set/Mapの判別が絡む）・`parsePipeRHS`の引数列（`_`プレースホルダーの追跡が絡む）の3箇所は、要素ごとの判断ロジックが`parseOne`という単純な形に収まらないため、同じ「closeに達するまでループ・要素の後とカンマの後にskipNewlines」というパターンを手書きのまま踏襲するに留めた（統一より重複の方が読みやすいと判断——原則3の「1つの仕組みで足りるものを2つ用意しない」は構文の一貫性についての話であり、内部実装の共有まで強制するものではないと判断した）。
2. **`parseFieldCallArgs`は`parseCommaList`が返す`nil`（要素0件）を空スライス`[]ast.StructLitField{}`へ変換してから返す**——この関数の戻り値は「`nil`＝`.field`単体アクセス、非nilの空スライス＝`.field()`という0引数呼び出し」という既存の区別（`ast.FieldExpr.Args`のdoc comment、Step 14以来）を守る必要があり、`parseCommaList`の「空なら`nil`」という汎用的な挙動をそのまま漏らすと壊れる——ヘルパーを共有しつつ、この1箇所だけ呼び出し側で後処理を挟む形にした。
3. **`|>`の行またぎ継続は、括弧の深さ管理とは別の「先読みバッファ」で実現した**——パーサーに`buf []lexer.Token`（`advance()`が`lx.Next()`より先に読む）と`peekPastNewlines()`（改行を消費せずその先の最初の非改行トークンを覗く。覗くために追加で読んだトークンは`buf`へ積まれ、後続の`advance()`が透過的に消費する）を新設した。`parsePipeExpr`の`pipeContinues()`ヘルパーが、`p.cur`が改行なら`peekPastNewlines()`で先を覗き、`|>`ならその改行群を`skipNewlines()`で確定的に消費してから継続、`|>`でなければ**何も消費せず**改行を残す（`parseBlock`側の文区切り処理がそのまま働けるように）——「覗いてから決める」を、消費してしまうと後戻りできない`advance()`だけでは実現できないため、この先読みバッファが必須だった。`|>`は他のどの式の先頭にもなり得ない（あらゆる値・キーワードと衝突しない専用トークン）ため、「次の非改行トークンが`|>`かどうか」だけで継続/非継続を一意に判定できる——曖昧性が原理的に生じない設計。
4. **`x[a:b]`のスライス境界にも同じ`skipNewlines()`を素朴に追加した**——これはカンマ区切りリストではないが、`[`から`]`まで自分自身の閉じトークンを持つという点で(1)と同じ「曖昧性が無い」条件を満たすため、ついでに対応した（需要は低いが、コストもほぼ無かった）。
5. **`(...)`/`[...]`/structリテラルの`{...}`の中で改行を許すようにしても、ブロック本体の`{ }`の改行の意味（文区切り）とは一切干渉しない**——事前に懸念していた「`{`の多重役割」問題は、実装してみると発生しなかった。理由は`parseCommaList`等がbracket/paren/brace記号そのものの対応（開いた記号と同じ種類の記号でしか閉じない）に依存しており、`(...)`の中に`{ ... }`ブロック（クロージャー本体）が現れても、そのブロックは`parseBlock`（改行を文区切りとして扱う、全く別の関数）が独立してパースするため、"括弧の深さ"を跨いだ状態の混線が起こりようがなかった——クロージャーリテラル`fn(x: Int) -> Int { x * 2 }`を呼び出し引数の中に書ける場面（ex4のパイプ右辺、あるいは将来インライン引数として許可された場合）でも、`{ x * 2 }`の内部は`parseBlock`が独立して処理するので、外側の`parseCallArgs`が改行を無視する設定になっていることと衝突しない。

**実装は`internal/parser/parser.go`のみ**（`internal/ast`/`internal/sema`/`internal/codegen`は無変更）——構文糖衣・パース戦略の変更に完全に閉じており、AST形状もセマンティクスも一切変わらないため（Step 9の`|>`糖衣展開と同種の「パーサーだけで完結する」変更）。新しいAMIVM命令も不要。

**実地検証（`amivm`→`go build`→実行）で確認した項目**：複数行の`List`リテラル(末尾カンマ込み)・複数行`|>`チェーン(`for...yield`のyield式内も含む)・複数行タプルリテラル・複数行`Set`/`Map`リテラル・複数行structリテラルを1つのプログラムに詰め込み、`amifl run`で最後まで通ることを確認（`total=6, len(doubled)=3, tupleSum=6, len(set)=3, len(map)=2, p.x+p.y=3`）。加えて、通常の1行区切りの2文（`let x = 5` の次行に `x`）が引き続き2つの独立した式としてパースされる（新しい先読みロジックが誤って次の行を飲み込まない）ことも確認した。`internal/parser/parser_test.go`に6件のテストを追加（複数行`List`+末尾カンマ・複数行呼び出し引数+末尾カンマ・`|>`の行またぎ継続・行またぎ継続が無関係な次の文を飲み込まないことの否定的確認・複数行タプル/structリテラル+末尾カンマ）。`gofmt`/`go vet`/`go test`/`make test-examples`はすべてgreenで、既存`examples/`全ファイルに回帰が無いことも確認済み。`amifl-spec.md` §5・`tour/01-scalars-and-variables.md`・`tour/06-pipe-and-errors.md`も実装に合わせて更新した（前者2つは「1行1式」という説明を「ブロック直下の文区切りのみ改行が効く／閉じトークンを持つリストは複数行可」という正確な説明へ差し替えた）。

### ロードマップ9番目の拡張：`&&`/`||`の短絡評価（ex9）——AND/OR命令の廃止とIF/ELSEへの置き換え、および実装中に発見した無関係な既存バグ（discard時のBinaryExpr/UnaryExprの副作用消失）の修正

`amifl-spec.md`（刷新前から）は`&&`/`||`が短絡評価**ではない**ことを明示的な既知の限界として記載していた（旧§17.2#1）——`genBinaryValue`が常に両オペランドを`genValue`で無条件評価してからAMIVMの`AND`/`OR`命令（両オペランドがすでに計算済みのbool値であることを前提とする、素朴な二項演算命令）へ渡す実装だったため。ユーザーからの明示的な依頼で短絡評価へ切り替えた。

**確定した設計判断**：

1. **`genBinaryValue`は`Op`が`&&`/`||`のときだけ、新設の`genShortCircuitValue`へ分岐する**——AMIVMの`AND`/`OR`命令はもう使わない（`binaryInstr`/`binaryResultIsBool`マップから両エントリを削除した）。`genShortCircuitValue`はStep 11dの`genMinMaxValue`等が確立した「`IF`/`ELSE`で分岐ごとに`SET`する」パターン（CLAUDE.mdの既存コードに複数の先例あり）をそのまま踏襲：`a && b`は`IF a { tmp = b } ELSE { tmp = false }`、`a || b`は`IF a { tmp = true } ELSE { tmp = b }`という2命令列に展開する。右オペランド（`b`）の`genValue`呼び出しは対応する分岐の中でのみ行われるため、その中に埋め込まれた副作用（関数呼び出し等）も自然に短絡評価の対象になる——AMIVMの`IF`/`LOOP`が本物のネストしたGoブロックを生成する（Step 4以来確立済みの前提）ため、コード生成側で特別な巻き上げ処理は一切不要だった。
2. **`a && b && c`のような左結合の連鎖・`a && b || c`のような優先順位混在も、既存の再帰構造だけで正しく短絡する**——`genValue`が任意の式に対して`genBinaryValue`を再帰的に呼ぶ既存の仕組みにより、`(a && b)`自体もBinaryExprである以上、それを右オペランドとして持つ外側の`||`から見ても、内側の`&&`の評価（またはスキップ）は自動的にその属するIF/ELSE分岐の中に閉じ込められる——新しい再帰ロジックを一切書く必要が無かった（実地に`a && b || c`の3項連鎖で確認済み、下記参照）。

**実装中に発見した無関係な既存バグ（discard時にBinaryExpr/UnaryExprの副作用が消えていた）**：`genShortCircuitValue`を書くために`genStmt`（`_ = expr`および文位置のUnit強制式の実行を担う）を読み直したところ、`*ast.BinaryExpr`と`*ast.UnaryExpr`が`*ast.IntLit`等の「無条件に副作用が無い」種別と同じ分岐にまとめられ、`_ = f() + g()`のような**演算子の被演算子に副作用のある呼び出しが混ざる式を捨てると、その呼び出し自体が一切実行されない**という、Step 3以来（関数呼び出しが被演算子になり得るようになったStep 5以来、実質的には）ずっと潜在していたバグを発見した——実際に`_ = sideEffect(1) + sideEffect(2)`が2つの`print`をどちらも一切実行しないことを`amifl run`で再現確認した。Step 3の「確定した設計判断」（`_ = expr`は演算子式に対しても無条件で何もIRを生成しない）は、当時（リテラルとconst参照しか被演算子になり得なかった時点）は正しかったが、以後の拡張がこの前提の崩れを見落としていた——「ロジック上正しそうに見えるだけで次のステップへ進まない」という原則が、今回は自分が今書いているコードではなく、何年も前の確定済みコードに対して事後的に効いた形。対策は`BinaryExpr`/`UnaryExpr`を`IntLit`/`FloatLit`/`BoolLit`/`StringLit`/`IdentExpr`（これらは被演算子を持たず、本当に無条件で純粋）とは別の分岐に切り出し、`TupleLit`/`StructLit`等と同じ「`genValue`で値を組み立ててから結果を捨てる」経路へ回すことで修正した——今回のex9の作業とは独立した、ついでに見つけて直したバグ。
...
### ロードマップ11番目の拡張：`return`の導入（ex11）——コンパイラ内部限定の`Never`型を新設し、`break`/`continue`も含めて「if/switchの分岐として他の実型と混在できる」よう一般化した。`?`演算子は`return`の特殊化として併存を継続、削除しなかった

Step 4の`resolveBreakExpr`/`resolveContinueExpr`は当時から「Never型が実装されたら見直すこと」という自己言及コメントを残していた（"revisit once `return`/Never's design is settled for real"）。ex11でその宿題を回収する形で着手した。

**確定した設計判断**：

1. **`Never`型（`neverType = "Never"`、`internal/sema/types.go`）を新設し、`return`/`break`/`continue`の3つ全てをこれに解決させた**——`checkExpr`（`internal/sema/expr.go`）の型不一致検査に「`typ == neverType`なら`expected`が何であっても素通しする」という1行のバイパスを追加するだけで、「制御が発散する式は、周囲がどんな型を期待していても文句を言わせない」という中核の意味論が実現できた。`break`/`continue`はStep 4以来ずっと`unitType`を返しており、Unit位置（ループ本体の通常の文）としては何の問題も無かったが、if/switchの分岐としては使えなかった（Step4の同コメントが明示的に認めていた制約）——`Never`へ切り替えるだけで、既存のUnit位置での使い方には一切影響を与えずに、分岐としての使用も同時に解禁できた。
2. **`checkBlock`（`internal/sema/sema.go`）のブロック非最終式チェックも同じ理由で`t != unitType && t != neverType`へ緩和した**——`if x { break }`のような、ループ内の非最終文としての既存の使い方が壊れないよう、Unitチェックの緩和もセットで必要だった。
3. **`resolveBranches`/`resolveSwitchExpr`（if/elif/elseとenum switchの分岐型統一ロジック、それぞれ別実装だが同じ「非リテラル分岐を優先して型のアンカーにする」パターンを持つ）を、共有ヘルパー`pickBranchAnchor`へ統合し、3段のフォールバック（(1)非リテラル・非Never分岐を最優先 → (2)非Never分岐（リテラルでも可） → (3)全分岐がNeverならbranch 0）へ拡張した**——`if done { return 5 } else { 10 }`で`10`側がリテラルなのにNever分岐（`return 5`）がアンカーに選ばれてしまうと、`target`が`Never`になり肝心の`10`側が型を持てなくなる、という具体的なバグを実装中に発見し、この3段構成に至った（詳細は`amifl_implementation_notes.md`参照）。全分岐がNeverの場合（両方`return`等）は式全体がNeverになる——「本当に値を生成しない」という状況を正しく表現できる。
4. **`return`/`break`/`continue`は文の位置（`parseExpr`）でのみパース可能とし、`parsePrimaryExpr`（値表現の入口、呼び出し引数・二項演算子の被演算子などから再帰的に到達可能）からは完全に排除した**——理由は意味論的な制約ではなくAMIVM-IRの構造的な制約：3つとも単一のジャンプ系命令（`RET`/`BREAK`/`CONTINUE`）へ下がる以上、他の命令の引数スロットへ「ジャンプの値」を埋め込む方法がAMIVM-IRに存在しない。この制限は実は`break`/`continue`にも遡って適用されるべきものだった——**Step 4時点で`break`/`continue`は`parsePrimaryExpr`から到達可能なまま残っており、`f(break)`のような構文が（Unit型がほとんどの実引数型と一致しないため）偶然無害だっただけの、当時気づかれていなかった潜在バグだった**——ex11の`Never`型化でUnitチェックが外れたことで、この構文が急に「型検査を通ってしまうが、`genValue`に`*ast.BreakExpr`のケースが無くコード生成が失敗する」という顕在化したバグになるところだった（実装中に発見・対処、詳細は`amifl_implementation_notes.md`参照）。対策として`break`/`continue`も`parsePrimaryExpr`から`parseExpr`（文位置）へ移設した——ブロックの最終文（tail）は依然として「文」として扱われるため、`if done { return 5 } else { 10 }`のような分岐としての用途は損なわれない。呼び出し引数だけは別枠で`parseCallArgs`が`parseExpr`ではなく`parsePipeExpr`を使うよう変更し、この経路を完全に塞いだ。
5. **`let`/代入（プレーン代入・`x[i]=v`・`p.field=v`の3形式）は、値が*直接* Never型である場合をsemaで明示的に拒否した**——`readAssignableContainer`等のcodegenが最終的に`genValue`を呼ぶ以上、値がNeverだと「値が無いのにSET/ASET/FSETしようとする」という構造的に无意味な状態になる。`const`は既存の`requireConstExpr`（許可するAST形状のホワイトリスト）が`*ast.ReturnExpr`等を最初から受け付けないため無風で安全、`_ = return 5`は`DiscardExpr`のcodegenが`genValue`ではなく`genStmt`を経由する設計（Step 2以来）のおかげで実は安全（`RET`が直接発行されるだけ）——この3+2種の経路を全て実地検証で確認してから、安全でない4箇所（let・plain assign・index-assign・field-assign）にだけ`typ == neverType`の拒否を追加した。「ブロックの直接の値としてはダメだが、if/switchの分岐の中でならOK」という非対称性は、`genBlock`/`genValueBlock`（下記）が分岐の中でのNeverを特別扱いできることの裏返し。
6. **codegen側は`genBlock`（関数/クロージャー本体の末尾）と`genValueBlock`（if分岐/switchケースの末尾）の両方に、末尾が`return`/`break`/`continue`かどうかを判定する`isDivergentExpr`チェックを追加し、該当する場合は`genValue`ではなく`genStmt`を呼ぶよう分岐させた**——`genValue`はNever型の式に対応するケースを一切持たない（到達不能なので追加しない、Step 5のClosureLit「到達不能なケースを書かない」という既存方針の踏襲）。分岐が発散する場合、その分岐用に確保された結果一時変数（`dest`）へは何も`SET`しない——制御はそこへ到達する前に必ず発散するため、未設定のままで問題ない（Goのコンパイラは経路ごとの未代入検査を通常のローカル変数には行わないため、これは合法）。
7. **`?`演算子は削除しなかった**——ユーザーから「`?`が不要になったなら削除」という条件付きの依頼だったが、`return`の導入後も`?`は`Tuple2[U,Error]`/`Error`伝播という最頻出パターンに特化した明確な価値を持ち続ける：`?`が1トークンで行う「ペイロードのFGET・ペイロード型のゼロ値構築・Error側のFSET・条件RET」を`return`だけで書こうとすると、呼び出し側が毎回ペイロード型のゼロ値を手で組み立てる必要があり（`if isError(e) { return (0, e) }`のように、Intなら`0`で正しくてもstructやTupleなら書き手が正しいゼロ値を知っている必要がある）、大幅に冗長になる。原則3（1つの仕組みで足りるものを2つ用意しない）は「`?`と`return`が同じ問題を別々に解いている」ケースには当てはまらない——`?`は`return`の上に成り立つ特殊化されたシンタックスシュガーであり、既存の`emitEarlyReturn`（`internal/codegen/errors.go`）のコード自体も`genReturnStmt`とは完全に独立したまま、変更せずに残した（両者は最終的に同じ`RET`命令へ収束するが、コード生成経路としては最後まで別物）。

**実地検証（`amivm`→`go build`→実行）で確認した項目**：`examples/control_flow.aml`に3つのヘルパー関数（`earlyIfNegative`——値ありの早期return、`firstOverThreshold`——`for`ループ内からの早期return、`describeSign`——if分岐としての`return`と実型の混在）を追加し、`main`から呼び出して終了コードへ合流させた（159、既存139に20を加算）。加えて、使い捨てプログラムで以下を個別に確認：値ありの早期return・値なしの早期return（Unit関数）・`for`ループ内からの早期return・`if`分岐としての`break`（ループ内の`while`で）・全て正しい評価順序と値で動作（`amivm`→`go build`→実行、print順序と最終終了コードの両方を手計算と照合）。ネガティブケースとして、`return`をUnit以外の関数で値省略時に使う・`return`の値の型不一致・`f(return 1)`（呼び出し引数位置）・`5 + return 1`（二項演算子の被演算子位置）・`let x = return 5`（letの直接の値）・`x = return 5`（代入の直接の値）・`xs[i] = return 5`・`p.field = return 5`の8パターンがいずれも明確なエラーメッセージでコンパイルエラーになることを確認した。`internal/parser/parser_test.go`に7件、`internal/sema/sema_test.go`に10件、`internal/codegen/codegen_test.go`に3件のテストを追加。`gofmt`/`go vet`/`go test`/`make test-examples`はすべてgreenで、既存`examples/`全ファイルに回帰が無いことも確認済み。`amifl-spec.md`の2.2/3.3/4/5/7/8.1節・17.1節（`return`/`Never`型の行を旧版差異一覧から削除——実装と一致したため）を更新した。

### ロードマップ12番目の拡張：`write`が`String`を直接受け付けるようにした（ex12）——新しい変換関数を追加するのではなく、既存の組み込み関数`write`自身をcapability多相化する形で解決した

Step 12完了時点で、`write(f: File, data: Bytes) -> Tuple2[Int, Error]`は`Bytes`（`List[UInt8]`）専用で、文字列をファイルへ書くには数値のバイト列を`List[UInt8]`型注釈付きで手で綴るしかなかった（`amifl-spec.md`の既知の限界として`tour/`にも明記されていた）。ユーザーからの依頼は「ファイルへ文字列を書くのを簡単にする。関数提供など方法は任せる」——具体的な設計は一任された。

**確定した設計判断**：

1. **新しい名前の関数（`writeString`等）を追加するのではなく、既存の`write`自身を`String`/`Bytes`のcapability多相にした**——原則3（1つの仕組みで足りるものを2つ用意しない）に照らし、`contains`/`reverse`/`concat`（13.4節）が既に確立している「第1引数（または該当引数）の静的型を見て、その場でcapabilityグループを判定し`v.ArgTypes`へ記録する」という設計パターンをそのまま踏襲した——`resolveWrite`（`internal/sema/builtins_chan.go`）が`v.Args[1]`の型を無条件チェックで読み取り、`String`か`List(UInt8)`（`Bytes`の正規型）のどちらでもなければ拒否する。呼び出し側は`write(f, "text")`と`write(f, bytesValue)`のどちらも同じ関数名で書け、ユーザーが「今どちらの経路を通っているか」を意識する必要が無い。
2. **codegen側は`c.ArgTypes[1]`を読んで、`amiflrt.WriteFile`（既存、`[]byte`引数）と新設の`amiflrt.WriteString`（`string`引数）のどちらを呼ぶか分岐するだけで済んだ**——`WriteString(fh, s) { return WriteFile(fh, []byte(s)) }`という1行のGo関数をamiflrtへ追加し、文字列→バイト列の変換をGo側に完全に閉じ込めた。AMIVM-IR側で`[]byte`への型変換を試みる案（`CALL %tmp : ?[]byte %s`）も検討したが、`[]byte`は`callname`オペランドカテゴリが要求する「裸の識別子」の形を持たない複合型なので、そもそも書けない構文だった——amiflrt側に変換ロジックを置く一択だった。
3. **`Bytes`が`List[UInt8]`への完全なエイリアスである（Step 12の既存設計）ことのおかげで、Bytes側の経路は一切変更不要だった**——既存の`amiflrt.WriteFile(fh, data []byte)`は、AmiFLの`Bytes`値（Goのネイティブ`[]uint8`スライス、named型）をそのまま渡せる（Goの代入可能性規則：片方が無名型なら同一underlying typeどうしは変換不要、`[]byte`は無名型）。変更が必要だったのは「`String`という新しい入力形を追加で受け付ける」分岐だけで、既存の`Bytes`経路（`genWriteValue`の元々のコード）は1行も触れていない。

**実地検証（`amivm`→`go build`→実行）で確認した項目**：使い捨てプログラムで、`write(f, "Hello, ")`+`write(f, "AmiFL!\n")`（String2回書き込み）→ 読み戻して内容とバイト数が一致することを確認（`amivm`→`go build`→実行、書き込みバイト数`n1=7, n2=7`・読み戻しの`len(content)=14`が一致）。既存の`Bytes`経路（`write(f, listOfUInt8)`）も無変更で動作することを別途確認。`write(f, 5)`（Int、どちらの型でもない）が明確なエラーメッセージで拒否されることも確認した。`examples/streams_io.aml`の3箇所の`Bytes`リテラル書き込みのうち2箇所を素直な`String`リテラルへ置き換え（終了コード38は変わらず、バイト列として同一の内容であることが実地で裏付けられた）、1箇所は`Bytes`経路の実演として意図的に残した。`internal/sema/sema_test.go`の`TestCheck_WriteRequiresBytesArg`（`write`へ`String`を渡すと拒否される、という**もう成り立たなくなった**前提のテスト）を`TestCheck_WriteAcceptsStringArg`へ置き換え、`TestCheck_WriteRequiresStringOrBytesArg`（Int等の無関係な型は依然拒否される）を新設。`internal/codegen/codegen_test.go`に`TestGenerate_WriteWithStringArgCallsWriteString`を追加。`gofmt`/`go vet`/`go test`/`make test-examples`はすべてgreenで、既存`examples/`全ファイルに回帰が無いことも確認済み。`amifl-spec.md` 13.10節の`write`シグネチャを更新し、`tour/08-concurrency-and-io.md`・`tour/10-wrapup.md`の「String→Bytes変換関数が無い」という既知の限界の記述を削除・更新した（ついでに、tour執筆(6aa1407)以降にex9/ex10で古くなっていた`tour/01-scalars-and-variables.md`・`tour/02-control-flow.md`・`tour/05-tuples-structs-enums.md`・`tour/06-pipe-and-errors.md`の短絡評価/フィールド代入/`return`に関する記述もこの機会に一括して追随させた——ロードマップの各拡張がtourの更新を毎回セットで行うべきだったところ、ex9/ex10では抜けていたことに気づいた）。

### ロードマップ13番目の拡張：`let (a, b, ...) = value`によるタプル分解代入（ex13）——新しいASTノードを1つ足すだけで、既存の`let`・タプルフィールドアクセスの仕組みを一切変えずに実現できた

ex12完了後、ユーザーから「`let value, err = result.0, result.1`を一気に代入する方法を追加。構文はおまかせ。Tuple2〜Tuple8全てに対応」という依頼で着手。構文の選定だけがこの回の設計判断で、それ以外は既存の仕組みの組み合わせで完結した。

**確定した設計判断**：

1. **構文は`let (a, b, ...) = value`——LHSをタプルリテラルと同じ`(...)`で囲む形にした**——ユーザー提案の`let value, err = ...`（カンマ区切り、括弧無し）も検討したが、`(...)`で囲む形の方が「タプルを作るときの構文（`(1, "a")`）とタプルを分解するときの構文が同じ見た目になる」という対称性を持ち、他言語（Rust・ML系）でも定着している見慣れたパターンであること、かつ`let`直後のカンマ区切り名前列だけでは「複数の`let`をカンマで並べて書ける」という別の意味に読めてしまう曖昧さが無いことから、括弧付きの形を選んだ。パーサー側の実装コストもほぼ同じ（`let`の直後が`(`かどうかで1トークン先読みするだけ）だったため、構文上の分かりやすさを優先した。
2. **`ast.LetTupleExpr`という新しいASTノードを1つだけ追加し、既存の`ast.LetExpr`には一切手を入れなかった**——「1つの仕組みで足りるものを2つ用意しない」（原則3）という基準に照らすと一瞬迷うが、`LetExpr`は「1つの名前・省略可能な型注釈・1つの値」という形を最初から前提にしており、複数名前・複数トークン・タプル要素ごとの型という形はこの構造に無理やり詰め込むより新設した方が素直だった——Step 10の`for`が「1変数形」と「2変数形（Map専用）」を同じ`ast.ForExpr`に`Var`/`Var2`として同居させた前例はあるが、あちらは「変数がたかだか2個」という上限が最初から決まっていたのに対し、こちらは「Tuple2〜Tuple8」で2〜8個と可変長になるため、`Names []string`という配列を持つ別ノードにする方が構造として素直だった。
3. **名前ごとの型注釈は実装しなかった**——`let (a: Int, b: String) = pair`のような書き方は許可していない。Step 5の`ClosureLit`が「クロージャー自身の引数・戻り値が既に完全に明示されているため、`let`側の型注釈は本質的に冗長」として省いた判断（8.3節）と全く同じ理由——タプルの型は`value`の解決済み型（`checkExpr(v.Value, "")`、期待型を渡さない）から一意に決まるため、各名前への注釈は常に重複情報にしかならない。
4. **`_`をある位置に書くとその要素を明示的に捨てられる、という設計にした**——ユーザーの依頼には無い追加機能だが、`Tuple2[T, Error]`から欲しいのは片方だけ、という場面（例えば`let (value, _) = parse[Int](s)`）は`parse`/`okOr`/`unwrap`の使用感からして頻出すると判断し、既存の`_ = expr`（`ast.DiscardExpr`）が持つ「値を捨てる」という意味をそのまま流用した——新しい記号や別の捨て方を発明せず、AmiFLに既にある「`_`は捨てる印」という語彙を再利用しただけなので、原則3にも反しない。sema（`resolveLetTupleExpr`）は`_`の位置で`fc.declare`を呼ばずスキップするだけ、codegen（`genLetTupleStmt`）は`Tokens[i] == ""`の位置でVAR/FGETを丸ごと出さないだけ——「宣言も生成もしない」というシンプルな実装で済んだ。
5. **`value`は必ず1回だけ評価する**——`genLetTupleStmt`は`genValue(v.Value)`を1度だけ呼び、その結果トークンへ位置ごとに`FGET`を重ねるだけ（Step 6の`genFieldValue`が確立した「target を`genValue`で1回評価してから複数箇所のFGET元にする」という既存パターンをそのまま踏襲——新しいアイデアではなく、既存のtarget評価規約をそのまま複数フィールドへ拡張しただけ）。`value`が`parse[Int](s)`のような副作用のある呼び出しであっても、名前の数だけ再実行されることはない。
6. **各名前の宣言は既存の`fc.declare`をそのまま呼ぶだけで実現し、重複名検出・シャドーイングのロジックを一切書いていない**——`let (a, a) = t`のような同一スコープ内の重複名は、`fc.declare`が既存の「同じスコープで同名再宣言はエラー」というルール（Step 2以来）でそのまま拒否される。新しい重複検出コードを1行も書かずに済んだ。

**実装は4ファイル**：`internal/ast/ast.go`（`LetTupleExpr`ノード新設）、`internal/parser/parser.go`（`parseLetExpr`冒頭の`(`分岐、`parseLetTupleExpr`——名前列は既存の`parseCommaList`をそのまま使い、複数行・末尾カンマもex8の仕組みを通じて無償で対応できた）、`internal/sema/expr.go`（`resolveLetTupleExpr`——`checkExpr`→`tupleTypeParts`での要素分解→`fc.declare`のループ、という3手順のみ）、`internal/codegen/codegen.go`（`genLetTupleStmt`——`genValue`1回→位置ごとのVAR+FGET）。新しいAMIVM命令は不要——既存の`VAR`/`FGET`のみで完結した。

**実地検証（`amivm`→`go build`→実行）で確認した項目**：使い捨てプログラムで、`let (n, s) = pair()`（Tuple2、関数呼び出しの結果を分解）・`let (a, b, c) = triple()`（Tuple3）・`let (value, _) = pair()`（`_`による位置の破棄）・`let (parsed, err) = parse[Int]("99")`（`Tuple2[Int, Error]`を返す組み込み関数の結果を直接分解）を1つの`main`にまとめ、終了コード189（`42+1+2+3+42+99`）が手計算と一致することを確認。生成されたIRを目視確認し、`pair()`の呼び出し結果が一時変数へ1回だけCALLで束縛され、そこから`F0`/`F1`への個別のFGETが生成されていること（呼び出しの再実行が無いこと）、`_`の位置（2回目の`pair()`呼び出しのF1側）にはFGETが一切出力されないことを確認した。加えて、名前の個数とタプルの要素数の不一致（3個の名前に対し2要素タプル）・非タプル値の分解（`let (a, b) = 5`）・重複名（`let (a, a) = (1, 2)`）・トップレベルでの使用（構造的な文法エラーになること、`LetExpr`と同じ）の4パターンを手動でCLIから確認し、いずれも期待どおり明確なエラーメッセージ/構文エラーになることを確認した。`examples/tuples_structs.aml`に`let (px, py) = (7, 8)`・`let (_, keep) = (0, 99)`を追加し、終了コード205（既存91に`px(7)+py(8)+keep(99)=114`を加算）が手計算と一致することを確認。`internal/parser/parser_test.go`に3件、`internal/sema/sema_test.go`に5件、`internal/codegen/codegen_test.go`に1件（値を1回だけ評価すること・`_`位置にFGETが出力されないことの両方を検証）の回帰テストを追加。`gofmt`/`go vet`/`go test`/`make test-examples`はすべてgreenで、既存`examples/`全ファイルに回帰が無いことも確認済み。`amifl-spec.md`の4節・冒頭注記、`tour/05-tuples-structs-enums.md`・`tour/06-pipe-and-errors.md`も実装に合わせて更新した。

### ロードマップ14番目の拡張：`extern`のローカル`.go`ファイル経路（ex14）——仕様書15.3節に「意図された設計」として書かれていたが、実装が一度も追随していなかった食い違いを発見・解消した

tour/examplesのドキュメント総点検を依頼された流れで、ユーザーがGemini Notebook製のインタラクティブガイド（`amifl-spec.md`を読み込んだもの）に「`.aml`と同じ場所に`.go`ファイルを置き、`extern`で束ねれば使える」と言われたが本当か、と確認を求めてきた。調査したところ、これは**Gemini Notebookの幻覚ではなく、`amifl-spec.md` 15.3節が最初からそう書いていた**（「同じディレクトリに手書きの`.go`ファイルを置き、そこで定義した関数を`extern`で束ねる、という15.1節と同じ経路に統一する」）——しかし実際の`cmd/amifl/build.go`を読むと、`extern "path"`の`path`はamivmの`-i alias=path`へ**そのまま**渡され、`amifl build`自体は`.aml`ソースのディレクトリとは完全に切り離された一時ディレクトリ（`os.MkdirTemp`、`module amiflbuild`だけの空の`go.mod`＋`amiflrt/`のコピー）の中でビルドしており、ローカルの`.go`ファイルをそこへコピーする処理が1行も無かった——つまり15.3節は「仕様として書いたが実装されなかった」項目が、全面刷新後の現行`amifl-spec.md`自体に紛れ込んでいた、というCLAUDE.md冒頭が警告する事態そのものだった（旧版特有の問題だと思われていたが、今回初めて刷新後の本体にも起こりうると判明した）。

**確定した設計判断**：

1. **`extern "path"`の`path`が`.`・`./...`・`../...`で始まるかどうかで「ローカルの.goファイル」と「Go標準ライブラリ/モジュールのインポートパス」を区別する**——これはGo自身が`go build ./foo`と`go build github.com/x/y`を区別する慣習そのものであり（真のGoインポートパスは`.`から始まらない）、AmiFL独自の新しいマーカー構文を発明する必要が無かった。しかも`import alias "./x"`（12.2節）が既に採用している「参照元ファイル自身のディレクトリからの相対パス」という規約と完全に同じ形なので、ユーザーが2つの異なる相対パス構文を覚える必要も無い（原則3の「1つの仕組みで足りるものを2つ用意しない」を、構文設計レベルでも徹底できた）。
2. **解決先ディレクトリ直下の`*.go`ファイルを非再帰的に集める**——1つのAmiFLパッケージが「1つのディレクトリの直下だけ」を見る12.1節の規則（サブディレクトリは別パッケージ）と対称的な設計にした。ディレクトリが存在しない・ディレクトリでない・`*.go`ファイルを1つも含まない場合は、`amivm`や`go build`の分かりにくいエラーとしてではなく、`cmd/amifl`側の時点で明確なメッセージのエラーとして止める。
3. **コピー先の`.go`ファイルが宣言する`package`名は一切気にしない**——生成コードは常に`extern`の`as`エイリアスを明示的なGoインポートエイリアス（`import alias "..."`）として使うため、Goの「宣言されたパッケージ名とインポート先で使う識別子は無関係にできる」という言語機能にそのまま乗れる。`amiflrt`自身が`import amiflrt "amiflbuild/amiflrt"`という形で、`amiflrt.go`が宣言する`package amiflrt`と（たまたま一致してはいるが）本質的には独立に解決されているのと同じ理屈——ユーザーに命名規約を強制する必要が無かった。
4. **同じ絶対ディレクトリを指す複数の`extern`宣言（同じパッケージ内で別名を2つ付ける、あるいは異なるAmiFLパッケージからそれぞれ相対パスで同じ場所を指す）は、コピーを1回だけ行い使い回す**——`externMap`の値を「生の文字列」ではなく`externImport{Alias, Path, LocalDir}`という構造体にし、ローカルパスの場合は解決済みの絶対ディレクトリを比較対象にすることで、`import alias "path"`が既に持っていた「同じエイリアスが異なるターゲットに束縛されたら別パッケージ間でもエラー」という衝突検出ロジック（12.4節と同型の規則）を、生の文字列比較のままでは正しく機能しない場面（相対パスの綴りが違っても実体が同じ）でも壊さずに済んだ。
5. **codegen（`internal/codegen`）には一切手を入れなかった**——`ExternImportMappings`は今まで通り宣言された生の`alias=path`文字列を返すだけの純粋関数のまま（ファイルシステムに触れない、CLAUDE.mdのリポジトリ構成が定める「semaとcodegenはastにのみ依存する」という層構造を保つ）。ローカルパスの判定・解決・コピーは全て`cmd/amifl/build.go`（唯一ファイルシステムI/Oを行う層）に閉じ込めた——`amiflrt`自身を一時ディレクトリへコピーする`copyAmiflrt`と全く同じ設計上の居場所に、新しい`copyLocalExternDir`を追加しただけ。
6. **`sema`側の検証も変更していない**——`registerExternTypes`は元々`d.Path == ""`しか検査しておらず、`.`や`./foo`という文字列はそのまま非空文字列として素通りする。ローカルパスかどうかの意味論はビルドパイプライン（ファイルシステムを実際に見る必要がある判断）の領分であり、静的検査だけで完結するsemaの責務には含まれないと判断した。

**実装は`cmd/amifl/build.go`1ファイルのみ**：`externImport`構造体（`Path`か`LocalDir`のどちらか一方だけを持つ）・`isLocalExternPath`・`resolveLocalExternDir`（`pkg.Dir`基準で解決、存在確認込み）・`copyLocalExternDir`（非再帰的に`*.go`だけをコピー、空なら明確なエラー）を新設し、`compileToIR`の返り値を`[]string`から`[]externImport`へ変更、`compileToGo`側で一時ディレクトリ作成後に一意なローカルディレクトリごとへ`extern0`, `extern1`, ...という連番のサブディレクトリへコピーしてから、最終的な`-i alias=path`引数列を組み立てる。新しいAMIVM命令もamivm本体への機能要求も不要——既存の`-i alias=path`の仕組みと、`amiflrt`が確立した「スクラッチモジュール配下へコピーしてローカル解決させる」パターンの組み合わせだけで完結した。
