...
Step 3時点で、以下の命令だけで足りた。**実際に`amivm`→`go build`→実行まで通して動作確認済み。**
...
Step 3で追加した後半（算術〜文字列連結）は`examples/operators.aml`が一括で実証している——自己検算式（演算子ごとの計算結果を積み上げ、期待値との差分をプロセスの終了コードへ埋め込む設計）になっており、`go build`→実行→終了コードの数値一致確認、という1本のテストで全命令をカバーしている。単項マイナス（`NEG`相当）に対応する命令はAMIVMに無く、代わりにリテラルへの符号直接付与（命令ゼロ）または`SUB 0 x`への展開（下記§1）で表現した。
